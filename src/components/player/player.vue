<template>
  <div class="player" v-show="playList.length>0">
    <transition name="normal">
      <div class="normal-player" v-show="fullScreen" @click="back">
...
// import {prefixStyle} from 'common/js/dom' 这里有问题需要修改
export default {
  mixins: [playerMixin],
  data() {
    return {
  ...
  computed: {
    cdCls() {
      return this.playing ? 'play' : 'play purse'
    },
    playIcon() {
  ...
  methods: {
    enter(el, done) {
      ...
      let animation = {
        0: {
          transform: `translate3d(${x}px, ${y}px, ${scale}px)`
        },
        60: {
    ...
    onProgressBarChange(percent) {
      ...
      this.$refs.audio.currentTime = currentTime // currentTime IS A READ AND WRITE PROPERTY
      if (!this.playing) {
        this.togglePlaying()
      }
      if (this.currentLyric) {
    ...
    getLyric() {
      this.currentSong.getLyric().then((lyric) => {
        this.currentLyric = new Lyric(lyric, this.handleLyric) // this hanlder called when lineNum change
        if (this.playing) {
